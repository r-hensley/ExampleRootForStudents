...
void signal_FFT(char const *rootfile = "../data/1000evn_v3.root", int nevn = 100)
{
	...
	TH1F *hit_difference = new TH1F("hit_difference", "Ch 0 relative to first of Ch 0;Time Difference;Magnitude", nt_2, t_min_2, t_max_2);
	...
	for(int i= 0; i< nevn; i++) {  
		...
		for(int j =0; j< nsum_ch ;j++) {
		   
			event_time = revent->GetVPeakSumTime1()[j] * 0.001;
			
			if (event_time < t_min || t_max < event_time) {
				continue;
			}
